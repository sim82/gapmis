...
void wrap ( const char * s1, const char * s1_header, const char * s2, const char * s2_header, const char * diff, int len, FILE * output )
 {
   ...
   if ( j < m || j < n ) 
    {
      print_line ( s1, i * len, GM_min ( m, n ), &nr_gaps_a, ( i + 1 ) * len, 0, output, s1_header );
      print_line ( diff, i * len, ( m < n ) ? m : n, NULL, ( i + 1 ) * len, 1, output, NULL );
      print_line ( s2, i * len, GM_min ( n, m), &nr_gaps_b, ( i + 1 ) * len, 0, output, s2_header );
    }
   ...
   if ( out -> where == 1 ) //gap is in the text
    {
      ...
      wrap ( p, p_header, seq_gap, t_header, mark_mis, GM_LINE_LNG, output ); 
    }
   else                     //gap is in the pattern
    {
      print_alignment ( p, m, t, n, seq_gap, mark_mis, in, out );
      wrap ( seq_gap, p_header, t, t_header, mark_mis, GM_LINE_LNG, output ); 
    }
         ...
         if ( out -> where == 1 ) //gap is in the text
          {
            ...
            wrap ( *p, *p_header, seq_gap, *Tmp_header, mark_mis, GM_LINE_LNG, output ); 
          }
         else                     //gap is in the pattern
          {
            print_alignment ( *p, m, *Tmp, n, seq_gap, mark_mis, in, out );
            wrap ( seq_gap, *p_header, *Tmp, *Tmp_header, mark_mis, GM_LINE_LNG, output ); 
          }
