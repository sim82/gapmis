   ...
   if ( j < m || j < n )
    {
      ...
      for ( i = 0; i < nr_lines; ++ i )
       {
         print_line ( tmp -> data + sizeof ( char ), j + i * len, j + ( i + 1 ) * len, ( m < n ) ? &nr_gaps_a : &nr_gaps_b, j + ( i + 1 ) * len, 0, output, tmp -> header );
       }
      if ( j + i * len < k )
       {
         print_line ( tmp -> data + sizeof ( char ), j + i * len, k, ( m < n ) ? &nr_gaps_a : &nr_gaps_b, j + ( i + 1 ) * len, 0, output, tmp -> header );
       }
...
void wrap2 ( struct TSeq * s1, struct TSeq * s2, const char * diff, int len, FILE * output )
 {
   ...
   if ( j < m || j < n ) 
    {
      print_line ( s1 -> data, i * len, min ( ( i + 1 ) * len, n ), &nr_gaps_a, ( i + 1 ) * len, 0, output, s1 -> header );
      print_line ( diff, i * len, ( m < n ) ? m : n, NULL, ( i + 1 ) * len, 1, output, NULL );
      print_line ( s2 -> data, i * len, min ( ( i + 1 ) * len, m), &nr_gaps_b, ( i + 1 ) * len, 0, output, s2 -> header );
    }
   ...
   if ( j < m || j < n )
    {
      ...
      for ( i = 0; i < nr_lines; ++ i )
       {
         print_line ( tmp -> data, j + i * len, j + ( i + 1 ) * len, ( m < n ) ? &nr_gaps_a : &nr_gaps_b, j + ( i + 1 ) * len, 0, output, tmp -> header );
       }
      if ( j + i * len < k )
       {
         print_line ( tmp -> data, j + i * len, k, ( m < n ) ? &nr_gaps_a : &nr_gaps_b, j + ( i + 1 ) * len, 0, output, tmp -> header );
       }
